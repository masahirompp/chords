...
class Clef {
  ...
  private static GClefDef:IClef = {
    ...
    width: 44,
    offset: -3.5, // scale考慮済みの値
    scale: 0.5
  };

  private static FClefDef:IClef = {
    id: 'FClef',
  ...
  private clefDef:IClef;
  private _ratio:number;

  // private
  constructor(public cType:ClefType, ratio?:number) {
    this.clefDef = cType == ClefType.GClef ? Clef.GClefDef : Clef.FClefDef;
    this._ratio = ratio || 1;
  }

  get id():string {
    return this.clefDef.id
  }

  get path():string {
    return this.clefDef.path;
  }

  get scale():number {
    return this.clefDef.scale * this._ratio;
  }

  get width():number {
    return this.clefDef.width * this.scale;
  }

  get offset():number {
    return this.clefDef.offset * this._ratio;
  }

  public static factory(id:String, ratio?:number):Clef {
    if(id === 'GClef') {
      return new Clef(ClefType.GClef, ratio);
    }
    return new Clef(ClefType.FClef, ratio);
  }
